...
| S8 | Setup/subrepo refresh integration | `scripts/setup-agent-subrepos.sh`, `ready_position` | `000` partial | Deferred | None in repo tests | GAP (medium) |
...
1. Add dedicated spec/tests for `S6` (human queue ingestion and escalation).
2. Add dedicated spec/tests for `S7` (notification channels and delivery failure behavior).
3. Add an integration harness or mocked git tests for `S8` (`scripts/setup-agent-subrepos.sh`) to cover partially-initialized subrepo states.
4. Improve `S10` prompt artifact generation policy coverage with focused fixtures.
